...
def main():
	...
	print "Initialized" 
        ...
        while True:
		    ...
		    clock_speed.labels(device=i).set(clock_info.real)
		    log.info('Clock statistics = %s', str(clock_info))
				
		    power_value = nvmlDeviceGetPowerUsage(handle)
		    power_usage.labels(device=i).set(power_value.real/1000.0)
		    log.info('Power Usage statistics = %s', str(power_value))
					
		    temperature_value = nvmlDeviceGetTemperature(handle,0)
		    temperature.labels(device=i).set(temperature_value)
		...
		print hostname
		print core.REGISTRY
                push_to_gateway('100.98.24.211:9091', job=hostname, registry=core.REGISTRY)
